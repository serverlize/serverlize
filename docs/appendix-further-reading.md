---
id: appendix-further-reading
title: Further Reading
---

## DynamoDB

### Pagination

- [“Guys, we’re doing pagination wrong…”](https://hackernoon.com/guys-were-doing-pagination-wrong-f6c18a91b232)

### Filtering

- [Comparison Operator and Function Reference](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.OperatorsAndFunctions.html)
- [Condition Expressions](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Expressions.ConditionExpressions.html)

## Inspiration

### AWS

- [`cloudform`](https://github.com/bright/cloudform)

### Monorepo

- [Monorepo setup with Lerna and Yarn workspaces](https://medium.com/trabe/monorepo-setup-with-lerna-and-yarn-workspaces-5d747d7c0e91)
- [Running Jest Tests Before Each Git Commit](https://benmccormick.org/2017/02/26/running-jest-tests-before-each-git-commit/)
- [`lerna-yarn-workspaces-example`](https://github.com/Quramy/lerna-yarn-workspaces-example)
- [`lint-staged-multi-pkg`](https://github.com/sudo-suhas/lint-staged-multi-pkg)
- [`serverless-mono-example`](https://github.com/tommedema/serverless-mono-example)
- [`typescript-workspace-plugin`](https://github.com/spion/typescript-workspace-plugin)

## Libraries

- [`accesscontrol`](https://github.com/onury/accesscontrol)
- [`TypeStrict`](https://github.com/krzkaczor/TypeStrict)
- [`lambda-warmer`](https://github.com/jeremydaly/lambda-warmer)
- [`@fluffy-spoon/substitute`](https://github.com/ffMathy/FluffySpoon.JavaScript.Testing.Faking)
- [`collect.js`](https://github.com/ecrmnn/collect.js)
- [`lint-staged`](https://github.com/okonet/lint-staged)
- [`release/semantic-release`](https://github.com/semantic-release/semantic-release)
- [`middy/tree/1.0.0-alpha`](https://github.com/middyjs/middy/tree/1.0.0-alpha)
- [`commitlint`](https://github.com/marionebl/commitlint)
- [`cz-cli`](https://github.com/commitizen/cz-cli)
- [`lerna-semantic-release`](https://github.com/atlassian/lerna-semantic-release)
- [`ts-jest`](https://github.com/kulshekhar/ts-jest)

## TypeScript

- [TypeScript 2.4: Dynamic import() Expressions](https://blog.mariusschulz.com/2018/01/14/typescript-2-4-dynamic-import-expressions)
- [Dynamic import expressions](https://basarat.gitbooks.io/typescript/docs/project/dynamic-import-expressions.html)
- [Exhaustive Switches in TypeScript](http://ideasintosoftware.com/exhaustive-switch-in-typescript/)
- [What's new in TypeScript](https://github.com/Microsoft/TypeScript/wiki/What's-new-in-TypeScript)
- [Announcing TypeScript 3.0](https://blogs.msdn.microsoft.com/typescript/2018/07/30/announcing-typescript-3-0/)
- [Typescript dependency injection: setting up InversifyJS IoC for a TS project](https://itnext.io/typescript-dependency-injection-setting-up-inversifyjs-ioc-for-a-ts-project-f25d48799d70)

## Parameter Store

- [AWS Parameter Store](https://www.davehall.com.au/blog/dave/2018/08/26/aws-parameter-store)
